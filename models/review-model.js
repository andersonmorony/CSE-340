...
async function addReview(review_text, inventory_id, account_id){
  ...
  async function getReviewByInvId(review_id) {
    try {
      const sql = "SELECT review_id, review_text, review_date, R.inv_id, R.account_id, A.account_firstname, A.account_lastname, A.account_type, I.inv_make, I.inv_year, I.inv_model FROM review as R LEFT JOIN account as A ON R.account_id = A.account_id LEFT JOIN inventory as I ON R.inv_id = I.inv_id WHERE R.review_id = $1";
  ...
  async function updateReview(review_id, review_text){
    try {
      ...
      const result = pool.query(sql, [review_text, review_id])
      return result.rows[0]
    } catch (error) {
      return error.message
    }
  ...
  module.exports = { addReview, getReviewByInvId, getReviewByAccount_id, updateReview}